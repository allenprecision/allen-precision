...
class ShopifyInstance(models.Model):
    ...
    def _fetch_all_shopify_customers_by_email(self, session, shop_url):
        """Bulk-fetch every Shopify customer (paginated, 250/page) and index
        by lowercased email. One-time cost of a handful of calls instead of
        one Shopify search request per Odoo candidate — that per-record
        search is what made this slow on stores with many customers.
        ...
        params = {"fields": "id,first_name,last_name,email", "limit": 250}
